...
public class Customisation : MonoBehaviour
{
    ...
    [SerializeField]
    private GameObject nameTaken;
    [SerializeField]
    private GameObject pointsUnused; 
    ...
    public void Save()
    {
        ...
        database.strengthIndex = player.stats[0].value;
        database.dexterityIndex = player.stats[1].value;
        database.constitutionIndex = player.stats[2].value;
        database.wisdomIndex = player.stats[3].value;
        database.intelligenceIndex = player.stats[4].value;
        database.charismaIndex = player.stats[5].value;

        database.slotIndex = PlayerData.saveSlot;

        database.SavingToDataBase(ProcessResultofRequest);
        //SceneManager.LoadScene(2);
    }

    public void ProcessResultofRequest(string result)
    {
        Debug.Log(result);
        if (points != 0)
        {
            pointsUnused.SetActive(true);
        }
        else
        {
            Debug.Log(result);
